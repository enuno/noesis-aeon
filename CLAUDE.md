...
4. **Irreversible side-effects — prefer in-run.** The default for an auth'd irreversible action (email, image gen, ad spend) is to run it **in-run** via `./secretcurl`, as the skill's *final*, fail-closed action — so a failure surfaces in the same run instead of a detached step. A skill *may* instead defer to the on-success gate — write a request file to `.pending-{service}/` for an operator-provided `scripts/postprocess-{name}.sh` that runs **after a successful run** — but only when "runs after the whole run succeeded" is a genuine requirement (e.g. `deploy-prototype`'s repo-create + Vercel deploy). **Never** defer a read.
