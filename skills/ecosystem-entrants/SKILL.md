...
category: meta
description: Diff ECOSYSTEM.md against the last run — surfaces newly-added projects (and removed ones) as a discrete weekly signal so new entrants aren't lost in the static list
var: ""
...
Today is ${today}. `ECOSYSTEM.md` is the curated list of projects, agents, and products building on top of Aeon. New rows arrive in irregular bursts — multiple ecosystem PRs can land within seconds of each other. Each new entrant is a potential co-marketing partner, integration target, or community member worth following up with. `ecosystem-pulse` tracks the **liveness** of projects already in `ECOSYSTEM.md`, but nothing surfaces new arrivals as a discrete weekly signal. At a healthy contribution velocity, new projects arrive faster than a human scanning the PR queue would catch.
...
|-------|-------------------|---------|------|
| `ecosystem-pulse` | "Are listed projects shipping this week?" | Weekly (Mon) | Liveness of *existing* entries |
| **`ecosystem-entrants`** | **"What was added to ECOSYSTEM.md this week?"** | **Weekly (Mon, slightly later)** | **First-touch of *new* entries** |
...
| `gh api search/issues?q=...` | Best-effort match of "added row" to a merged PR in the current repo for the notification's "added by" link | `GH_TOKEN` |
...
```json
{
  ...
  "last_status": "OK",
  "entries": {
    "https://x.com/exampleproject": {
      "name": "exampleproject",
      "primary_url": "https://x.com/exampleproject",
      "links": [{"label": "@exampleproject", "url": "https://x.com/exampleproject"}],
      "logo_url": "https://...",
...
Resolve the current repo once (never hardcode an owner):

```bash
REPO=$(gh repo view --json nameWithOwner -q .nameWithOwner)
```

For each entry in `added` (skip on first/baseline run; skip when `MODE=dry-run` to keep the dry run hermetic), best-effort attribute the row to a merged PR in `${REPO}`:

```bash
gh api -X GET "search/issues" -f q="repo:${REPO} is:pr is:merged ECOSYSTEM.md in:title,body \"${name}\"" \
...
`OK` and `QUIET` are the two success states. The split lets the dashboard show "ran clean, nothing changed" without overloading the OK row — the same pattern `ecosystem-pulse` and `competitor-radar` use.
