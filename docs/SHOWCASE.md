...
A snapshot of the operators currently running their own Aeon instance. "Active" means the fork pushed within the last 30 days. Source: the `fork-fleet` divergence monitor against the GitHub forks API.
...
The full ranking — including the per-week skill-divergence buckets — is regenerated by the `fork-fleet` skill, with the latest output published to [`output/articles/`](../output/articles/).

### What forks teach upstream

The fleet is **infrastructure-first**: 18 of 24 active forks run only `heartbeat`. Of the rest, the most common second skills are `github-trending`, `morning-brief`, and the crypto cluster (`token-movers`, `price-alert`). When two or more forks independently flip the same default, the `fork-fleet` skill flags it as a `DEFAULT_FLIP` candidate and surfaces it for upstream consideration.
...
Running an Aeon fork that should be on this page? Open a PR adding a row to the **Active forks** table. The `fork-fleet` skill automatically picks up active forks from the GitHub API; this page links the human-readable summary.
